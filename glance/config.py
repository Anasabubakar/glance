...
def _load_env_files() -> None:
    for env_path in (Path.cwd() / ".env", Path.home() / ".glance" / ".env"):
...
def active_provider() -> str:
    """Pick the provider: explicit GLANCE_PROVIDER, else first key present,
    else local Ollama (free, no key)."""
    explicit = os.environ.get("GLANCE_PROVIDER")
    if explicit:
