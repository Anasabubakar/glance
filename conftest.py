...
#    rootdir conftest's directory), so `import glance` / `import organizer`
#    resolve when running pytest from anywhere in the project.
#
# 2) It relocates pytest's temp dirs out of %TEMP% (…\AppData\Local\Temp\…).
#    Glance' safety guard refuses to operate anywhere under "appdata" — that is
...
def pytest_configure(config):
    if not config.option.basetemp:
        config.option.basetemp = str(Path.home() / ".glance-pytest-tmp")
