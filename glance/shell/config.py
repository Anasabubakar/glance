...
class Config:
    ...
    # App
    hotkey: str = field(default_factory=lambda: os.getenv("GLANCE_HOTKEY", "ctrl+alt+m"))
    ...
    def llm_provider(self) -> str:
        ...
        """
        override = os.environ.get("GLANCE_ACTIVE_LLM", "").strip().lower()
    ...
    def save_env_values(self, values: dict) -> None:
        ...
        %LOCALAPPDATA%\\Glance\\.env (the app dir is read-only)."""
        env_path = _WRITABLE_ENV
        try:
            env_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def stt_provider(self) -> str:
        ...
        forced = os.getenv("GLANCE_STT", "").strip().lower()
    ...
    def get_ollama_model(self, kind: str = "vision") -> str:
        ...
        Reads runtime override from GLANCE_OLLAMA_VISION_MODEL /
        GLANCE_OLLAMA_TEXT_MODEL first, then the dataclass field, then the
        legacy single-model knob.
        """
        env_key = "GLANCE_OLLAMA_VISION_MODEL" if kind == "vision" else "GLANCE_OLLAMA_TEXT_MODEL"
