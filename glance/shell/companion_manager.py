"""
Central state machine for Glance Windows.
...
def _load_soul() -> str:
    ...
    if _SOUL_CACHE is None:
        try:
            ...
            _SOUL_CACHE = (
                "You are Glance, a voice-first Windows buddy next to the cursor. "
...
def _build_system_prompt(
    ...
    if quiz_mode:
        return f"""You are Glance, an interactive QUIZ TUTOR. The user has
...
# Questions that ask Glance to locate / click UI elements — triggers the
...
class CompanionManager(RoutingMixin, TourMixin, ActionsMixin, QObject):
    ...
    def __init__(self):
        ...
        # Cross-session memory + learned skills (~/.glance/memory.json)
        ...
        # Load user-created skills from skills/ + ~/.glance/skills/
        try:
            skills_pkg.load_all()
        except Exception:
            pass
    ...
    async def _warm_up(self):
        ...
        try:
            ...
            if hasattr(tts, "synth"):
                ...
                # act) is chosen, so Glance responds audibly within ~1s even though
    ...
    def _get_stt(self):
        if self._stt is None:
            ...
            print(f"[glance-debug] STT provider = {provider} "
    ...
    def on_hotkey_press(self):
        # Barge-in: if Glance is thinking or speaking, the hotkey cancels the
    ...
    def _handle_wake(self):
        ...
        # Hard kill-switch: GLANCE_WAKE_WORD=0 means the wake word can NEVER fire,
        # regardless of any toggle state (demo recording says "Glance" in narration).
        if os.environ.get("GLANCE_WAKE_WORD", "1") == "0":
            slog("HEAR", "wake word detected but GLANCE_WAKE_WORD=0 — ignored")
            return
        if self._state != AppState.IDLE:
            return
    ...
    def _begin_capture(self):
        ...
        print("[glance-debug] recording started — hold the hotkey, speak, release", flush=True)
        self._emit_state(AppState.LISTENING)

    def _filler_enabled(self) -> bool:
        return os.environ.get("GLANCE_FILLER", "0") == "1"

    def _maybe_start_filler(self):
        """Perceived-latency: after a short delay, play a pre-synthesized 'thinking'
        clip to cover the LLM gap. Opt-in (GLANCE_FILLER=1); killed the instant the
    ...
    def _streaming_stt_enabled(self) -> bool:
        ...
        # Only for the Deepgram path (needs the key); set GLANCE_STREAM_STT=0 to
        ...
        return (os.environ.get("GLANCE_STREAM_STT", "1") != "0"
    ...
    def _start_streaming_stt(self):
        ...
        try:
            ...
            print(f"[glance-debug] streaming stt start failed: {e}", flush=True)
            self._stt_session = None
    ...
    async def _end_capture_and_process(self):
        ...
        try:
            ...
            _dev = "?"
        print(f"[glance-debug] captured {len(pcm)} bytes (~{len(pcm)/32000:.2f}s) "
              f"rms={_rms:.4f} mic='{_dev}'", flush=True)
        if 0 <= _rms < 0.002 and len(pcm) > 16000:
            print("[glance-debug] -> that's SILENCE from a live mic — the default "
                  "input device is probably wrong (Camo/Phone Link virtual mic?). "
                  "Settings > Sound > Input, pick your real mic, restart Glance.",
                  flush=True)
        if len(pcm) < 3200:  # < 0.1s of audio — ignore
            if session is not None:
                try:
                    ...
                    pass
            print("[glance-debug] -> dropped: too short / no audio. Hold the key while "
        ...
        try:
            ...
            if session is not None:
                try:
                    ...
                    transcript = ""
                if transcript.strip():
                    print("[glance-debug] STT via live stream", flush=True)
            if not transcript.strip():
                transcript = await self._get_stt().transcribe(pcm)
            slog("HEAR", f"{transcript!r}")
            if not transcript.strip():
                print("[glance-debug] -> dropped: empty transcript (audio captured, but "
            ...
            _t = transcript.strip()
            if (re.fullmatch(r"(hey[\s,]+)?(glance[\s,]+)?(un[- ]?do( that| it)?|"
            ...
            # Set GLANCE_STREAM_TTS=1 for per-sentence streaming on long replies.
            ...
            # GLANCE_STREAM_TTS=0 to force the old whole-reply pass.
            stream_tts = (os.environ.get("GLANCE_STREAM_TTS", "1") != "0"
            ...
            import traceback
            print("[glance-debug] pipeline error:", flush=True)
            traceback.print_exc()
