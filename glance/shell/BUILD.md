# Building Glance.exe for Windows

Two ways to package Glance for your friend's laptop:

| Method | Output | Size | Install | Best for |
|---|---|---|---|---|
| **Portable folder** | `dist\Glance\` | ~400-600 MB | Copy the folder, double-click `Glance.exe` | Quick testing, USB stick |
| **Setup installer** | `Setup-Glance.exe` | ~200-400 MB | Double-click, Next, Finish | Polished distribution |
...
```
dist\Glance\Glance.exe    ← hand this whole folder to your friend
```

Your friend:
1. Copies the `Glance` folder anywhere on their PC
2. (Optional) creates `.env` next to `Glance.exe` with their API keys — see `.env.example`
3. Double-clicks `Glance.exe`
4. Tray icon appears, Glance is running

**No Python needed on their machine.** Everything is bundled.

---

## Full installer (`Setup-Glance.exe`)
...
2. Run:
   ...
   build.bat installer
   ```
3. Output: `dist\Setup-Glance.exe` — a single self-extracting installer

Your friend runs `Setup-Glance.exe`:
- Pick install location (default: `C:\Program Files\Glance`)
...
When they launch `Glance.exe` the first time:
...
2. After `build.bat`, sign both files:
   ```bat
   signtool sign /f mycert.pfx /p PASSWORD /tr http://timestamp.digicert.com /td sha256 /fd sha256 dist\Glance\Glance.exe
   signtool sign /f mycert.pfx /p PASSWORD /tr http://timestamp.digicert.com /td sha256 /fd sha256 dist\Setup-Glance.exe
   ```
...
└── dist/
    ├── Glance/         ← portable folder — give this to friends
    │   ├── Glance.exe
    │   ├── _internal/  ← bundled Python + libs (~500 MB)
    │   ├── .env.example
    │   ├── LICENSE
    │   └── README.md
    └── Setup-Glance.exe ← single-file installer (if you built with installer flag)
```
