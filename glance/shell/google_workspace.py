...
Setup (one time):
  ...
  4. Download it as ``~/.glance/google_credentials.json``.
...
``~/.glance/google_token.json`` and auto-refreshed after that.
...
def setup_help() -> str:
    return (
        ...
        "save it as ~/.glance/google_credentials.json. Then run "
