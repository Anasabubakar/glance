...
🎬 **Demo & write-up:** [raynanwuyep.com/glance](https://raynanwuyep.com/glance) · ⬇ **[Download for Windows](https://github.com/Anasabubakar/glance/releases/latest)**

Glance is a voice-first desktop companion for Windows. Hold a hotkey, talk, and she:
...
| **Open source**        | OpenClicky has agent mode  | **empty — Glance fills this** |

Clicky is Mac-only. Glance brings the same idea to the majority of desktops that can't run it — open and free.
...
Keys: let the wizard collect them, or copy `.env.example` → `glance/shell/.env`:
...
The **file organizer** is move-only and fully reversible (`glance undo`). The **voice agent acts directly** — like Clicky, she does what you ask rather than nagging for permission — but she stops and hands back before genuinely irreversible, high-stakes actions (send, delete, buy). It's an early build acting on your real machine, so **watch her, and press `Esc` to stop at any time.**

## Roadmap

- **Learnable skills (SKILL.md)** — adopting the same open Agent Skills standard used by Claude, Hermes, and OpenClaw, with Glance's twist: you teach her by *voice*.
- **Glance Bridge (MCP)** — exposing her eyes and pointer as an MCP server, so *any* agent (Claude, OpenClaw, Hermes) can see and point at a Windows screen.
...
packaging/      # PyInstaller entry (glance.spec builds the .exe)
organizer/      # early file-organizer prototype — superseded by glance/, kept for its tests
```

## Credits & license

Glance is an independent project. It builds on ideas and open-source work from:
