...
Glance CLI.
    ...
    glance organize [PATH]          tidy a folder (default: Desktop)
    ...
    glance undo                     reverse the last organize
    glance --version

Autonomous by default (it just does it); every move is journaled so `glance
...
def _cmd_organize(args) -> int:
    ...
    try:
        ...
        print(f"Glance: organize failed via {provider_name}: {e}", file=sys.stderr)
        return 1
    ...
    if args.dry_run:
        ...
        if n:
            print("Run `glance undo` to reverse it.")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="glance", description="Glance — the agent you can take back.")
    p.add_argument("--version", action="version", version=f"glance {__version__}")
