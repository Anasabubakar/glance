...
- **Facts** are injected into every system prompt, so Glance always "knows"
...
class MemoryStore:
    def __init__(self, path: Optional[Path] = None):
        self.path = path or (Path.home() / ".glance" / "memory.json")
    ...
    def save(self) -> None:
        try:
            ...
            print(f"[glance-debug] memory save error: {e}", flush=True)
    ...
    def facts_block(self) -> str:
        """Prompt fragment listing what Glance remembers (empty string if none)."""
        if not self.facts:
            return ""
