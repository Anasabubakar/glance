"""
session_log.py — a timestamped flight recorder for a Glance turn.

Every meaningful event is logged with a wall-clock time AND an elapsed offset, to
the console and to ~/.glance/logs/session.log, so a whole turn can be replayed on
...
def slog(category: str, msg: str = "") -> None:
    """Log one timestamped event to console + ~/.glance/logs/session.log."""
    line = f"[{_stamp()}] {category:<6} {msg}"
    print(line, flush=True)
    global _fh
    try:
        if _fh is None:
            d = Path.home() / ".glance" / "logs"
