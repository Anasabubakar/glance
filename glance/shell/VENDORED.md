...
Per `docs/BUILD_PLAN.md`, Glance ships this as part of its own product (bundle,
...
`glance run` → `glance/companion.py` adds this directory to `sys.path` and runs
...
glance run                       # tray buddy; hold the hotkey and talk
```

Develop on the free local Ollama path (no API key needed for Phase 1; pointing
falls back to Windows UIA). See `glance/shell/SETUP.md` for provider keys.
