...
level:
    ...
    SKILL = {
        ...
        "description": "Lets Glance click for you instead of pointing.",
...
def _user_skills_dir() -> Path:
    """User-level skills dir at ~/.glance/skills/ — survives reinstall."""
    return Path.home() / ".glance" / "skills"


def load_all() -> list[dict]:
    """Discover + import every skill module in this package and ~/.glance/skills."""
    global _loaded
    _loaded = []

    # Bundled skills (shipped with Glance)
...
def _try_import(path: Path) -> None:
    try:
        spec = importlib.util.spec_from_file_location(
            f"glance_skill_{path.stem}", str(path)
        )
