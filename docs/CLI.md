...
No Claude Code, no agent SDK — Glance calls LLM providers directly, or runs
fully offline.
...
Glance auto-selects that provider; otherwise it defaults to Ollama. Override per
run with `-p`.
...
glance organize ~/Desktop --dry-run

# Preview with the zero-config sorter (no key, no model):
glance organize ~/Desktop -p heuristic --dry-run

# Do it (autonomous, but reversible):
glance organize ~/Desktop
...
Every organize writes a batch to `~/.glance/history/`. `glance undo` reverses the
