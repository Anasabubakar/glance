"""
organize_desktop.py — Glance skill: voice-driven desktop tidying with preview + undo.

Drop in your skills directory (Glance auto-loads it). Matches phrases like
...
NOTE: `manager` is Glance's CompanionManager. The attribute names below
(llm.complete, speak, ask_confirm) are placeholders — map them to whatever the
base actually exposes when you wire it in.
"""
...
def _make_llm_complete(manager):
    """Adapt Glance's active LLM provider into the planner's expected callable:
...
# ---- skill registrations (Glance's expected format) -----------------------
