"""
glance.companion — launches the vendored companion shell as Glance.

The shell (``glance/shell/``, vendored from Bitshank-2338/clicky-windows, MIT)
...
points. The computer-acting agent loop (``glance/agent/computer_loop.py``)
...
def launch() -> int:
    ...
    if getattr(sys, "frozen", False):
        try:
            import main as _shell_main  # bundled top-level; see glance.spec
    ...
    if not os.path.isfile(_ENTRY):
        print("Glance: companion shell not found at glance/shell/.", file=sys.stderr)
        return 1
