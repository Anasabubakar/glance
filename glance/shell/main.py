"""
Glance for Windows — Entry Point.
...
def _copilot_login_flow(tray, panel, manager):
    ...
    def _on_code(user_code: str, verification_uri: str):
        ...
        msg = (
            ...
            f"3. Click Authorize — Glance will sign in automatically."
        )
...
def main():
    ...
    manager.sig_error.connect(
        lambda e: tray.show_notification("Glance error", str(e))
    )
    ...
    def _on_doc_dropped(path: str):
        ...
        tray.show_notification(
            ...
            f"{path}\nAsk Glance about it now." if ok else
    ...
    def _wf_stop():
        ...
        if summary:
            tray.show_notification(
                "Workflow Captured",
                "Sent to Glance as context. Ask: 'what did I just do?'"
            )
    ...
    def _attach_doc():
        ...
        path, _ = QFileDialog.getOpenFileName(
            None, "Attach a document for Glance",
    ...
    def _save_diagnostics():
        ...
        try:
            ...
            providers_d = {}
        report = []
        report.append(f"Glance diagnostics — {datetime.datetime.now().isoformat()}")
    ...
    tray.show_notification(
        "Glance is running",
        f"Say 'Glance' or hold {cfg.hotkey}  |  LLM: {providers['llm']}",
    )
